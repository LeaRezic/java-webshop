...
// import {  } from '../../types/custom/global';
...
export interface IColumnAggregateFunction<V> {
  (values: V[]): V;
}
...
export interface IColumnCellFormatFunction {
  <T>(value: T, row: any): string | JSX.Element; // TODO consult @domagoj.cerjan about types here
}
export interface IColumnCellFormatFunction {
  <T>(value: T, row: any): string | JSX.Element; // TODO consult @domagoj.cerjan about types here
}
...
export interface IColumnConfig<T> {
  Cell?: any;
  Footer?: ComponentConstructor<any> | string;
  accessor?: DeepKeyOf<T> | IColumnAccessorFunction<T, any>;
  aggregate?: IColumnAggregateFunction<any>; // #TODO @bigd -> can this be typed correctly?
  ...
  formatter?: IColumnCellFormatFunction;
  minWidth?: number;
  maxWidth?: number;
  sortable?: boolean;
  style?: any;
  ...
  urlHandle?: boolean; // added for the link cell
}
...
export interface ICellInternalProps<T, V> {
  ...
  aggregate?: IColumnAggregateFunction<V>;
  dateTimeFormat?: string;
  formatter?: IColumnCellFormatFunction;
  field: DeepKeyOf<T> | string;
  label?: string;
