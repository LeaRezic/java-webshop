...
const defaultFilterMethod: IFilterFunction = (query, row, column) => {
...
export function getCellProps<T>(column: IColumnConfig<T>) {
  ...
  if (maybeAccessor != null) {
    if (Array.isArray(maybeAccessor)) {
      accessor = (data: any) => _.get(data, maybeAccessor);
  ...
  return {
    ...
    style: column.style,
    url: column.url,
    urlHandle: column.urlHandle,
  };
}

export function transformConfig<T>(columns: IRowConfig<T>): ReactTableColumnConfig {
  return columns.map((column) => {
    ...
    return {
      ...
      Footer: Footer ? (props: any) => <Footer { ...props } { ...cellProps } Cell={Cell}/> : null,
      Header: column.title,
...
export function mergeConfigs<T>(configA: IRowConfig<T>, configB: Array<Omit<IColumnConfig<T>, 'title'>>): IRowConfig<T> {
  ...
  for (const field of fieldsForA) {
    const cellConfigA = configA.find((cellConfig) => cellConfig.field === field);
    const cellConfigB = configB.find((cellConfig) => cellConfig.field === field);
